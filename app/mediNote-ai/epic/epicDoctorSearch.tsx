...
practitionerData
  ...
  const [practitioner, setPractitioner] = useState<EpicPractitioner | null>(null);
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      setError(null);
      const data = await APIService.searchEpicPractitioner(practitionerId.trim(), tokenId);
  ...
  return (
    <div className="epic-doctor-search bg-white rounded-lg shadow-lg p-6 max-w-md w-full mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <h3 className="text-lg font-semibold text-gray-900">Search Epic Practitioner</h3>
        <button
          onClick={onClose}
      ...
      <form onSubmit={handleSearch} className="space-y-4">
        <div>
          <label htmlFor="practitionerId" className="block text-sm font-medium text-gray-700 mb-2">
            Practitioner ID
          </label>
          <input
            id="practitionerId"
      ...
      {practitioner && (
        <div className="mt-6 p-4 bg-green-50 border border-green-200 rounded-md">
          <h4 className="text-sm font-medium text-green-800 mb-3">Practitioner Found</h4>
      ...
      {practitionerData && !practitioner && (
        <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-md">
          ...
          <p className="text-xs text-blue-600 mt-1">ID: {practitionerData.id}</p>
        </div>
      )}
    </div>
  );
