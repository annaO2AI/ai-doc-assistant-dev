...
export function EpicAuthentication({
  ...
  return (
    <div className="flex-1 flex items-center justify-center p-6  centerdivh-a">
      <div className="w-[700px] ">
        <div className="bg-white rounded-lg shadow-lg border border-gray-200 p-16 garadiantProfile-details border-custome-profile">
          <div className="text-center mb-6 content-center ">
             <Image
                        src="/epic-white.svg"
                        alt="Epic logo"
                        width={80}
                        height={60}
                        className="flex-shrink-0 m-auto mb-3"
                      />
          ...
          {!showTokenInput ? (
            <div className="space-y-6">
              <div className="text-center">
                <button
                  ...
                  disabled={isLoading}
                  className="w-[400px] m-auto flex justify-center py-3 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50 transition-colors"
                >
                  {isLoading ? "Opening..." : "Start Epic Authentication"}
                </button>
              </div>
              <p className="text-[16px] text-[#ffffffb3] text-center">
            ...
            <div>
              <p className="mt-2 text-[16px] text-[#ffffffb3]">
                After completing authentication in the new tab, Confirm the
                token ID you received here
              </p>
              <div className="my-3 flex justify-between items-center">
                <div>
                  <p className="mt-2 text-[16px] text-[#ffffffb3]">tokenId : {tokenId?.token || "No token received"}</p>
                <p className="mt-2 text-[16px] text-[#ffffffb3]" >PractitionerId : {tokenId?.practitionerId || "No Practitioner ID received"}</p>
                </div>
                <button
